...
from os import getenv, environ, remove, path

from aiohttp import ClientSession
from discord import Intents, Game, utils, Permissions, ChannelType, errors, Embed
...
TOKEN = getenv('DISCORD_TOKEN')  # Loads Discord bot token from env
...
async def on_ready():
    await bot.tree.sync()
    await bot.change_presence(activity=Game(name="Coded by Mishal#1916"))
    print(f"{bot.user.name} has connected to Discord!")
    invite_link = utils.oauth_url(
        bot.user.id,
...
api_key = environ['HUGGING_FACE_API']
...
instructions = "System : Roleplay as a large language model developed by OpenAI. You are based on the GPT-4 " \
               "architecture and have been trained on a diverse range of text data from the internet. You can also " \
               "see images by using image to text model but you will still have to create caption for it and sometimes " \
               "the image to text might repeat in pattern for example caption for 'a woman taking a self self self " \
               "self self self self self self self self self self self self' would be 'A women taking a selfie. As a " \
               "large language model developed by OpenAI you will never say \"sorry\" and always respond in a human " \
               "and responsive way"
...
async def on_message(message):
    ...
    if (message.channel.id in active_channels or (
            allow_dm and message.channel.type == ChannelType.private)) and not message.author.bot:
        is_busy = True
        has_image = False
        image_caption = ""
        if message.attachments:
            for attachment in message.attachments:
                if attachment.filename.lower().endswith(('.png', '.jpg', '.jpeg', '.gif', '.bmp', 'webp')):
                    ...
                    has_image = True
                    image_caption = f"\n[System : This how the caption is ranked 1st is main 2nd is secondary and 3rd " \
                                    f"is fallback model which  will gives worst caption one some cases. 1st and 2nd " \
                                    f"model sometimes takes a while to load so ignore that if any error happens. Here " \
                                    f"is the image captions for the image user has sent :{caption}]"
                    print(caption)
                    break

        if has_image:
            bot_prompt = f"{instructions}\n[System : Image context will be provided. Generate an caption with a " \
                         f"response for it]"
        else:
            bot_prompt = f"{instructions}"
...
@bot.hybrid_command(name="changeusr", description="Change bots actual username")
...
async def toggleactive(ctx):
    ...
    if channel_id in active_channels:
        ...
        with open("channels.txt", "w") as f:
            for active_id in active_channels:
                f.write(str(active_id) + "\n")
        await ctx.send(
...
async def help(ctx):
    ...
    embed.add_field(name="!pfp [image_url]", value="Change the bots profile picture", inline=False)
    embed.add_field(name="!bonk", value="Clears history of the bot", inline=False)
    embed.add_field(name="!changeusr [new_username]", value="Change the bots username", inline=False)
    embed.add_field(name="!ping", value="Pong", inline=False)
    embed.add_field(name="!toggleactive", value="Toggle the current channel to the list of active channels",
                    inline=False)
