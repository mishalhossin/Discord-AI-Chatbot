...
class Conversation():
    ...
    def GetList(self: type) -> List[Dict[str, str]]:
        ...
        return self.__model._session.get(
            f"{self.__model._API}/chat.loadTree,personas.listPersonas?batch=1&input={json.dumps(PAYLOAD)}",
            headers=self.__model._HEADERS).json()[0]["result"]["data"]["json"][0]["data"]
    ...
    def Rename(self: type, id: str, name: str) -> None:
        ...
        for cv in conversations:
            if id == cv["id"]:
                DATA_: object = self.__model._session.post(f"{self.__model._API}/chat.renameChat?batch=1", json=PAYLOAD,
                                                           headers=self.__model._HEADERS)

                if DATA_.status_code == 200:
                    self.__model._logger.debug(
                        f"Renamed conversation ({Fore.CYAN}{id}{Style.RESET_ALL}) to ({Fore.MAGENTA}{name}{Style.RESET_ALL}).")
                else:
